...
async function createProduct (req, res){
    ...
    const arrayImgs = req.files?.imgs;

    if(!req.files?.imgs) return res.status(500).send({message: 'La imagen es obligatoria'})
...
function getProduct(req, res){
    ...
    Product.findById(id, (e, product) => {
        ...
        if(e != null && e?.messageFormat == undefined) return res.status(404).send({message: 'El producto no existe'})
