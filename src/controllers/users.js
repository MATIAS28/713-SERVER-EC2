    ...
    User.findOne({email: email}, (err, user) => {
        ...
        if(err === null && user){
            bcrypt.compare(password, user.password, (err, result) => {
                ...
                if(result){
                    return res.status(200).send({id: user._id, token: jwt.createToken(user)});
                }else{
                    return res.status(500).send({message: 'Error al iniciar session'});
                }
            });
...
function addAddress(req, res){
    ...
    console.log(address)
    if(address.dni && address.phone_number && address.name && address.surname && address.address && address.neighborhood  && address.province && address.locality && address.post_code){
        console.log('hola')
        User.findByIdAndUpdate(req.user.sub, { $push: {addresses: address} }, (e, adressSaved) => {
            if(e) return res.status(500).send({message: 'Error al devolver la orden'})
            if(!adressSaved) return res.status(404).send({message: 'No fue posible agregar la direcciòn'})
